...
# What is JA3 ? 
  ...
  JA3 mechanism uses the client Hello packet to create a fingerprint. This fingerprint can be used to identify the operating system and the client from which the request was made. It can also be used to identify various commonly used malwares and avoid traffic from them to prevent your website. 
  ...
  Don't confuse this with the `User-Agent` which is sent in the request headers. A client can send anything in this as it is under the control on the user. Even if one sends a different `User-Agent` the source of the request can be found using the JA3
  fingerprint.

  Interesting fact! Have you ever tried to access google using tor browser and been blocked by a screen which says their systems have found unusual traffic, It it done by a mix of fingerprint filtering and by matching list of public tor exit relays. This way of filtering out traffic from certain client group is done by few govt sites also.
